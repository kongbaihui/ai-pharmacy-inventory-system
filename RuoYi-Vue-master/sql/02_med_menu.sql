...
-- 批次数量由入库、出库、盘点、过期清理等业务产生，不提供手工新增、修改、删除数量的按钮
insert into sys_menu (menu_name, parent_id, order_num, path, component, is_frame, is_cache, menu_type, visible, status, perms, icon, create_by, create_time, update_by, update_time, remark) values('批次效期维护', @menuId, 3, '#', '', 1, 0, 'F', '0', '0', 'system:batch:edit',   '#', 'admin', sysdate(), '', null, '仅允许修改有效期等效期信息');
insert into sys_menu (menu_name, parent_id, order_num, path, component, is_frame, is_cache, menu_type, visible, status, perms, icon, create_by, create_time, update_by, update_time, remark) values('库存总量重算', @menuId, 4, '#', '', 1, 0, 'F', '0', '0', 'system:batch:recalc', '#', 'admin', sysdate(), '', null, '按批次剩余数量重算库存总量');
